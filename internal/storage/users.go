...
func (s *Storage) IsUserExists(ctx context.Context, userID int64) (bool, error) {
	var exists bool
	err := s.DB.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM users WHERE telegram_id=$1)`, userID).Scan(&exists)
	return exists, err
}
...
func (s *Storage) GetUsername(ctx context.Context, userID int64) (string, error) {
	var username string
	err := s.DB.QueryRow(ctx, `SELECT username FROM users WHERE telegram_id=$1;`, userID).Scan(&username)
	if err != nil {
		return "", nil
	}
	return username, nil
...
func (s *Storage) GetPartnerUsername(ctx context.Context, userID int64) (string, error) {
	...
	err := s.DB.QueryRow(ctx, query, userID).Scan(&partner)
	if err != nil {
